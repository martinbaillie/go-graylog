// Code generated by go-swagger; DO NOT EDIT.

package system_messageprocessors

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"net/http"
	"time"

	"golang.org/x/net/context"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/runtime"
	cr "github.com/go-openapi/runtime/client"

	strfmt "github.com/go-openapi/strfmt"
)

// NewAvailableAdapterTypesParams creates a new AvailableAdapterTypesParams object
// with the default values initialized.
func NewAvailableAdapterTypesParams() *AvailableAdapterTypesParams {

	return &AvailableAdapterTypesParams{

		timeout: cr.DefaultTimeout,
	}
}

// NewAvailableAdapterTypesParamsWithTimeout creates a new AvailableAdapterTypesParams object
// with the default values initialized, and the ability to set a timeout on a request
func NewAvailableAdapterTypesParamsWithTimeout(timeout time.Duration) *AvailableAdapterTypesParams {

	return &AvailableAdapterTypesParams{

		timeout: timeout,
	}
}

// NewAvailableAdapterTypesParamsWithContext creates a new AvailableAdapterTypesParams object
// with the default values initialized, and the ability to set a context for a request
func NewAvailableAdapterTypesParamsWithContext(ctx context.Context) *AvailableAdapterTypesParams {

	return &AvailableAdapterTypesParams{

		Context: ctx,
	}
}

// NewAvailableAdapterTypesParamsWithHTTPClient creates a new AvailableAdapterTypesParams object
// with the default values initialized, and the ability to set a custom HTTPClient for a request
func NewAvailableAdapterTypesParamsWithHTTPClient(client *http.Client) *AvailableAdapterTypesParams {

	return &AvailableAdapterTypesParams{
		HTTPClient: client,
	}
}

/*AvailableAdapterTypesParams contains all the parameters to send to the API endpoint
for the available adapter types operation typically these are written to a http.Request
*/
type AvailableAdapterTypesParams struct {
	timeout    time.Duration
	Context    context.Context
	HTTPClient *http.Client
}

// WithTimeout adds the timeout to the available adapter types params
func (o *AvailableAdapterTypesParams) WithTimeout(timeout time.Duration) *AvailableAdapterTypesParams {
	o.SetTimeout(timeout)
	return o
}

// SetTimeout adds the timeout to the available adapter types params
func (o *AvailableAdapterTypesParams) SetTimeout(timeout time.Duration) {
	o.timeout = timeout
}

// WithContext adds the context to the available adapter types params
func (o *AvailableAdapterTypesParams) WithContext(ctx context.Context) *AvailableAdapterTypesParams {
	o.SetContext(ctx)
	return o
}

// SetContext adds the context to the available adapter types params
func (o *AvailableAdapterTypesParams) SetContext(ctx context.Context) {
	o.Context = ctx
}

// WithHTTPClient adds the HTTPClient to the available adapter types params
func (o *AvailableAdapterTypesParams) WithHTTPClient(client *http.Client) *AvailableAdapterTypesParams {
	o.SetHTTPClient(client)
	return o
}

// SetHTTPClient adds the HTTPClient to the available adapter types params
func (o *AvailableAdapterTypesParams) SetHTTPClient(client *http.Client) {
	o.HTTPClient = client
}

// WriteToRequest writes these params to a swagger request
func (o *AvailableAdapterTypesParams) WriteToRequest(r runtime.ClientRequest, reg strfmt.Registry) error {

	if err := r.SetTimeout(o.timeout); err != nil {
		return err
	}
	var res []error

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}
