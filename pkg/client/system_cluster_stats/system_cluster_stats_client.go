// Code generated by go-swagger; DO NOT EDIT.

package system_cluster_stats

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"
)

// New creates a new system cluster stats API client.
func New(transport runtime.ClientTransport, formats strfmt.Registry) *Client {
	return &Client{transport: transport, formats: formats}
}

/*
Client for system cluster stats API
*/
type Client struct {
	transport runtime.ClientTransport
	formats   strfmt.Registry
}

/*
Schema gets JSON schema of configuration class
*/
func (a *Client) Schema(params *SchemaParams, authInfo runtime.ClientAuthInfoWriter) (*SchemaOK, error) {
	// TODO: Validate the params before sending
	if params == nil {
		params = NewSchemaParams()
	}

	result, err := a.transport.Submit(&runtime.ClientOperation{
		ID:                 "schema",
		Method:             "GET",
		PathPattern:        "/system/cluster_config/{configClass}",
		ProducesMediaTypes: []string{"application/schema+json"},
		ConsumesMediaTypes: []string{""},
		Schemes:            []string{"http"},
		Params:             params,
		Reader:             &SchemaReader{formats: a.formats},
		AuthInfo:           authInfo,
		Context:            params.Context,
		Client:             params.HTTPClient,
	})
	if err != nil {
		return nil, err
	}
	return result.(*SchemaOK), nil

}

// SetTransport changes the transport on the client
func (a *Client) SetTransport(transport runtime.ClientTransport) {
	a.transport = transport
}
